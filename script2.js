document.addEventListener("DOMContentLoaded", function () {
  ...
  function displayInvestment() {
    personalContent.innerHTML = `
      <h3>Chức năng Đầu tư</h3>
      <p>Bạn có thể đầu tư vào chứng khoán, quỹ đầu tư phát triển, vàng, v.v.</p>
      <label for="investmentAmount">Nhập số tiền đầu tư (VND):</label>
      <input type="number" id="investmentAmount" placeholder="Nhập số tiền đầu tư">
      <button id="analyzeBtn">Phân tích đầu tư</button>
    ...
    analyzeBtn.addEventListener("click", function () {
      const investmentAmount = parseInt(
        document.getElementById("investmentAmount").value.trim()
      );
      if (!isNaN(investmentAmount) && investmentAmount > 0) {
        const profit = calculateInvestmentReturn(investmentAmount);
        document.getElementById(
          "investmentResult"
        ).innerHTML = `<p>Ước tính lợi nhuận: ${profit.toLocaleString()} VND</p>`;
      } else {
        document.getElementById(
          "investmentResult"
        ).innerHTML = `<p>Vui lòng nhập số tiền hợp lệ để đầu tư.</p>`;
      }
    });
  }

  function calculateInvestmentReturn(amount) {
    // Simple profit calculation for demonstration: 5% of investment
    return amount * 0.05;
  }
  ...
  function displayInteraction() {
    personalContent.innerHTML = `
      <h3>Chức năng Tương tác</h3>
      <p>Khám phá chiến lược quản lý quỹ của cộng đồng và nhận gợi ý cá nhân hoá dựa trên hành vi tài chính của bạn.</p>
      <button id="viewCommunityStrategiesBtn">Xem chiến lược cộng đồng</button>
      <button id="suggestStrategyBtn">Gợi ý chiến lược của tôi</button>
      <div id="strategyContent"></div>
    `;

    const viewBtn = document.getElementById("viewCommunityStrategiesBtn");
    viewBtn.addEventListener("click", function () {
      viewCommunityStrategies();
    });

    const suggestBtn = document.getElementById("suggestStrategyBtn");
    suggestBtn.addEventListener("click", function () {
      suggestStrategy();
    });
  }
  function viewCommunityStrategies() {
    const strategies = fetchCommunityStrategies();
    let displayStrategies = strategies
      .map((strategy) => `<p>${strategy.description}</p>`)
      .join("");
    document.getElementById("strategyContent").innerHTML = displayStrategies;
  }

  function fetchCommunityStrategies() {
    // Hàm này mô phỏng việc tải dữ liệu từ máy chủ
    return [
      {
        description:
          "Sử dụng phương pháp quỹ cân bằng để chia đầu tư đều cho cổ phiếu, trái phiếu, và vàng.",
      },
      {
        description:
          "Đầu tư 70% vào quỹ rủi ro thấp và 30% vào quỹ rủi ro cao để phát triển danh mục đầu tư đa dạng.",
      },
    ];
  }
  function suggestStrategy() {
    let strategyDescription = prompt("Mô tả chiến lược quản lý quỹ của bạn:");
    if (strategyDescription) {
      submitStrategy(strategyDescription);
      alert("Chiến lược của bạn đã được gửi để xem xét cộng đồng!");
    }
  }

  function submitStrategy(description) {
    // Trong thực tế, hàm này sẽ thực hiện một cuộc gọi API để gửi chiến lược của người dùng
  }
